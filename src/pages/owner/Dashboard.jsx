...
import { X, Calendar, DollarSign, Package, Users, TrendingUp, AlertTriangle, Search, Filter } from "lucide-react";
...
export default function Dashboard() {
    ...
    try {
      ...
      switch (type) {
        case "Clients":
          ...
          break;
        case "Staff Members":
          res = await axios.get("/auth/getstaff");
          break;
        case "Active Orders":
  ...
  const handleAlertClick = async (alertType) => {
    setModalType(alertType === "dueSoon" ? "Orders Due Soon" : "Pending Payments");
    setModalOpen(true);
    setModalLoading(true);
    setModalData([]);
    resetFilters();
    ...
    try {
      if (alertType === "dueSoon") {
        const res = await axios.get("/samples?dueSoon=true");
        setModalData(res.data);
      } else {
        const res = await axios.get("/purchase-orders?paymentPending=true");
        setModalData(res.data);
      }
    } catch (err) {
      console.error(`Failed to fetch ${alertType}:`, err);
      setModalData([]);
    } finally {
      setModalLoading(false);
    }
  ...
  const handleOrderClick = (order) => {
    ...
    window.location.href = `/owner/clients/${clientId}/${orderType}/${order._id}`;
  };
  ...
  return (
    <OwnerLayout>
      <div className="min-h-screen pb-10 space-y-10">
        ...
        {/* ================= DUE SOON ALERT ================= */}
        {!loading && alerts.dueSoon > 0 && showDueSoonAlert && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-amber-900">
                  ⚠️ {alerts.dueSoon} Order{alerts.dueSoon > 1 ? "s" : ""} Due Soon
                </h3>
          ...
          {loading
            ...
            : (
              <>
                <button
                  onClick={() => handleAlertClick("dueSoon")}
                  className="bg-white hover:bg-amber-50 rounded-3xl p-6 shadow-md transition-all text-left group cursor-pointer border-2 border-transparent hover:border-amber-200"
                >
                  <div className="flex items-start gap-3">
                ...
                <button
                  onClick={() => handleAlertClick("paymentPending")}
                  className="bg-white hover:bg-green-50 rounded-3xl p-6 shadow-md transition-all text-left group cursor-pointer border-2 border-transparent hover:border-green-200"
                >
                  <div className="flex items-start gap-3">
                ...
                {modalLoading ? (
                  ...
                  <div className="space-y-3">
                    {filteredModalData.map((item, idx) => (
                      ...
                      >
                        {/* Client or Staff Display */}
                        {(modalType === "Clients" || modalType === "Staff Members") && (
                          <div>
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 mt-2 text-sm flex-wrap">
                                ...
                                {item.paymentReceived !== undefined && (
                                  <span className="text-gray-500 flex items-center gap-1">
